...
# that can be used to create commands for managing this application 
...
# We have functions recreate_db() and seeder() that resets the DB where the recreate_db() functions deleted and then recreated the db and seeder() function re-enters the dummy data into the newly created database 
def recreate_db():
	db.drop_all()
	db.create_all()
	db.session.commit()
...
#  The recreate_db() and seeder() are then used in a function called rsd()
...
# This function can be used from the command line / terminal directly with the following command -
